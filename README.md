...
Telegram 是唯一人机入口，三个 cron 脚本各管一摊（轮询指令 / 状态汇总 / 实验看护），最终落到 Slurm 或项目仓库上；`sbatch` 包装器旁路把每次提交登记成实验文档。
...
完整参考（模块结构、命令行入口、项目配置、实验登记与看护、进度/ETA 说明）见 [COMMANDS.md](COMMANDS.md)。
